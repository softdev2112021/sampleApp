...
const getLocations = async (): Promise<ILocation[]> =>
  http
    .get(locationsURL)
    .then(({data: locationsApi}: {data: ILocationApi[]}) => locationsApi.length ? convertLocationsApi(locationsApi) : []
    );

const searchLocations = async (searchQuery: string): Promise<ISearchedLocation[]> => {
  return http
    .get(`${citiesURL}/${searchQuery}`)
    .then(({data: searchedLocations}: {data: ISearchedLocation[]}) => searchedLocations
    );
}
...
const addLocation = async (
  ...
  http
    ...
    .then(({data: locationApi}: {data: ILocationApi}) => convertLocationsApi([locationApi])
    );
...
export { getLocations, searchLocations, addLocation, deleteLocation, logIn, logOut };
